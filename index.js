...
const errorchannel = new WebhookClient({
  id: process.env.errorWebhookID,
  token: process.env.errorWebhookTOKEN
});
...
const allowedChannels = []; // Add your allowed channel IDs to this array or leave it like [] if you want to it to catch from all channels. Use , (comma) to separate more than one ID.
...
client.on('ready', () => {
  console.log("https://github.com/bipash28/Poketwo-Autocatcher - Made by bipash / BIPRO")
  ...
  const channel = client.channels.cache.get(process.env.spamChannelID)
  ...
  function spam() {
    const result = Math.random().toString(36).substring(2, 985);
    channel.send(result)
...
client.on('messageCreate', async message => {
  ...
  if (!isSleeping) {
    ...
    if (message.content.includes("Please tell us") && message.author.id === "716390085896962058") {
    ...
    } else if (message.content.startsWith("$say") && message.author.id == process.env.OwnerID) {
      let say = message.content.split(" ").slice(1).join(" ")
      message.channel.send(say)

    } else if (message.content.startsWith("$react") && message.author.id == process.env.OwnerID) {
      let msg
      try {
    ...
    } else if (message.content.startsWith("$click") && message.author.id == process.env.OwnerID) {
      ...
      if (message?.embeds[0]?.footer?.text.includes("Spawns Remaining")) {
        ...
        const embed = new EmbedBuilder()
                      .setTitle("**New Pokemon Caught!**")
                      .addFields({ name: "Caught in:", value: `<#collected.channel.id`, inline: true }, { name: "Pokemon Name:", value: name2, inline: true }, { name: "Pokemon Rarity:", value: rareity, inline: true })
                      .setFooter({ text: "Made by <@670459548166914048>" })
                      .setColor('#0099ff')
                      .setTimestamp();
          ...
          async function main() {
            try {
              const res1 = await ocrSpace(url, {
                apiKey: process.env.ocrSpaceApiKey
              });
              ...
              setTimeout(async () => {
                message.channel.send(`<@716390085896962058> c ${name}`)
                  .then(a => { }).catch(error => {
                    console.error(error);
                    const channel = client.channels.cache.get(process.env.errorChannelID)
                ...
                collector.on('collect', async (collected) => {
                  ...
                  if (collected.content.includes("Congratulations")) {
                    ...
                    logchannel.send({ embeds: [embed] }).then(b => { }).catch(error => {
